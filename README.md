...
参考链接 https://github.com/LihanChen2004/pb_rm_simulation

> 当前本地导航、战斗、裁判 LAN、出生区、实测结果及下一阶段任务，
> 统一记录在 [项目永久进度文档](.docs/PROJECT_STATUS.md)。本机完整编译最多使用 4 个 worker。
...
该独立入口启动红蓝两台轻量手动车、统一裁判、射线射击、碰撞伤害和 LAN
裁判端口，不改变原有 Fast-LIO + Nav2 启动流程。红蓝选手分别运行：

```bash
./install/rm_referee/lib/rm_referee/combat_teleop --robot red_robot
./install/rm_referee/lib/rm_referee/combat_teleop --robot blue_robot
```

完整规则、裁判 CLI 和验收方法见
[RM_COMBAT_CLI.md](.docs/RM_COMBAT_CLI.md)。
