...
def generate_launch_description():
    ...
    use_combat = LaunchConfiguration('combat')
    combat_control_mode = LaunchConfiguration('combat_control_mode')
    combat_autostart = LaunchConfiguration('combat_autostart')
    combat_lan = LaunchConfiguration('combat_lan')
    combat_lan_bind_host = LaunchConfiguration('combat_lan_bind_host')
    combat_lan_port = LaunchConfiguration('combat_lan_port')
    combat_red_token = LaunchConfiguration('combat_red_token')
    combat_blue_token = LaunchConfiguration('combat_blue_token')
    combat_referee_token = LaunchConfiguration('combat_referee_token')
    ...
    declare_combat_cmd = DeclareLaunchArgument(
        'combat',
        default_value='False',
        description='Enable logical projectile combat, armor target and referee')

    declare_combat_control_mode_cmd = DeclareLaunchArgument(
        'combat_control_mode',
        default_value='0',
        description='Combat control mode: 0=AUTO, 1=MANUAL')

    declare_combat_autostart_cmd = DeclareLaunchArgument(
        'combat_autostart',
        default_value='False',
        description='Start combat immediately instead of waiting for the referee')

    declare_combat_lan_cmd = DeclareLaunchArgument(
        'combat_lan',
        default_value='False',
        description='Expose the integrated referee LAN WebSocket gateway')

    declare_combat_lan_bind_host_cmd = DeclareLaunchArgument(
        'combat_lan_bind_host', default_value='0.0.0.0')
    declare_combat_lan_port_cmd = DeclareLaunchArgument(
        'combat_lan_port', default_value='8765')
    declare_combat_red_token_cmd = DeclareLaunchArgument(
        'combat_red_token', default_value='red-demo')
    declare_combat_blue_token_cmd = DeclareLaunchArgument(
        'combat_blue_token', default_value='blue-demo')
    declare_combat_referee_token_cmd = DeclareLaunchArgument(
        'combat_referee_token', default_value='referee-demo')
    ...
    start_combat = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(
            get_package_share_directory('rm_combat_gazebo'),
            'launch', 'rmul_combat_target.launch.py')),
        condition=IfCondition(use_combat),
        launch_arguments={
            'default_mode': combat_control_mode,
            'autostart': combat_autostart,
            'lan_gateway': combat_lan,
            'lan_bind_host': combat_lan_bind_host,
            'lan_port': combat_lan_port,
            'lan_red_token': combat_red_token,
            'lan_blue_token': combat_blue_token,
            'lan_referee_token': combat_referee_token,
        }.items(),
    )
    ...
    ld.add_action(declare_combat_cmd)
    ld.add_action(declare_combat_control_mode_cmd)
    ld.add_action(declare_combat_autostart_cmd)
    ld.add_action(declare_combat_lan_cmd)
    ld.add_action(declare_combat_lan_bind_host_cmd)
    ld.add_action(declare_combat_lan_port_cmd)
    ld.add_action(declare_combat_red_token_cmd)
    ld.add_action(declare_combat_blue_token_cmd)
    ld.add_action(declare_combat_referee_token_cmd)
